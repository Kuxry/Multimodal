...
# 读取数据
data_json_all = []
with open(MMDocIR_gt_file, 'r', encoding="utf-8") as f:
    for line in f:
        item = json.loads(line.strip())
        # 这里将 question_id 转成 int
        item["question_id"] = int(item["question_id"])
        data_json_all.append(item)

target_qids = {10696,11657}  # 用集合存储目标 ID，方便筛选
filtered_json = [item for item in data_json_all if item["question_id"] in target_qids]

print(f"过滤后可用的记录数: {len(filtered_json)}")
...
results = []
output_file = "mmdoc-11657-696.csv"
...
with torch.no_grad():
    ...
    for idx, item in enumerate(filtered_json):
